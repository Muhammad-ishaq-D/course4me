...
const RightSidebar = ({ courseName, plan, price, easyApply, date, courseId, scheduleId }) => {
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-black text-[#1C1C1C] leading-snug">
                {courseName || "Course"} – {plan || "Flexi+"}
              </p>
              {date && (
                <p className="text-[11px] text-gray-400 font-medium mt-0.5">{date}</p>
              )}
            </div>
            <div className="text-right shrink-0">
              {courseId && scheduleId && (
                <p
                  onClick={() =>
                    navigate(`/booking/packages?courseId=${courseId}&scheduleId=${scheduleId}`)
                  }
                  className="text-[10px] text-[#F15A24] font-bold cursor-pointer hover:underline"
                >
                  Change package
                </p>
              )}
              <p className="text-[14px] font-black text-[#F15A24]">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              <p className="text-xl font-black text-[#F15A24]">£{total.toFixed(2)}</p>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <p className="text-[12px] text-gray-600 leading-relaxed">
            I was very nervous before the course specially with the exam. But my
            trainer, Adam, was super friendly and{" "}
            <span className="text-[#F15A24] font-bold">
              really gave me the confidence I needed.
            </span>{" "}
            The mock exams on the app and course book{" "}
            <span className="text-[#F15A24] font-bold">
              helped me passed in the first attempt.
            </span>
          </p>
        </div>
