...
const ApplyModal = ({ job, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      {successOpen ? (
        <div className="bg-white relative w-full max-w-md rounded-3xl p-10 text-center shadow-[0_20px_60px_rgba(0,0,0,0.25)] animate-in fade-in zoom-in duration-300">
          {/* Close */}
          <button
            onClick={onClose}
            className="absolute right-6 top-6 font-bold text-[#364153] hover:text-black transition-colors"
          >
            <X size={20} />
          </button>
        ...
        /* Modal - The form container */
        <div className="bg-white w-full max-w-6xl max-h-[90vh] overflow-y-auto rounded-3xl p-8 relative shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
          {/* Close */}
          <button
            onClick={onClose}
            className="absolute right-6 top-6 font-bold text-[#364153] hover:text-black transition-colors"
          ...
          <div className="grid md:grid-cols-2 gap-10">
            ...
            <div className="space-y-5">
              ...
              <h3 className="font-bold text-[#2C3E50] flex items-center gap-2 pt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-[#2C3E50]">Do you have a valid SIA License? *</label>
                  <input placeholder="Yes / No" className="w-full border rounded-xl px-4 py-3 text-sm outline-none mt-1 focus:ring-2 focus:ring-[#F8510C] transition-all" />
                </div>
                <div>
                  <label className="text-sm font-medium text-[#2C3E50]">SIA License Number</label>
                  <input name="license" placeholder="Enter license number" onChange={handleChange}
                    className="w-full border rounded-xl px-4 py-3 text-sm outline-none mt-1 focus:ring-2 focus:ring-[#F8510C] transition-all" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-[#2C3E50]">Years of Experience *</label>
                  <input name="experience" placeholder="e.g. 2 years" onChange={handleChange}
                    className="w-full border rounded-xl px-4 py-3 text-sm outline-none mt-1 focus:ring-2 focus:ring-[#F8510C] transition-all" />
                </div>
                <div>
                  <label className="text-sm font-medium text-[#2C3E50]">Availability *</label>
                  <input name="availability" placeholder="e.g. Immediate" onChange={handleChange}
                    className="w-full border rounded-xl px-4 py-3 text-sm outline-none mt-1 focus:ring-2 focus:ring-[#F8510C] transition-all" />
                </div>
              </div>

              <div>
                <label className="text-sm font-medium text-[#2C3E50]">Cover Letter / Why are you suitable? *</label>
                <textarea name="cover" placeholder="Tell us why you're the perfect candidate..." onChange={handleChange}
                  className="w-full border rounded-xl px-4 py-3 text-sm outline-none h-28 mt-1 focus:ring-2 focus:ring-[#F8510C] transition-all" />
              </div>

              <div>
                <label className="text-sm font-medium text-[#2C3E50]">Upload CV (Optional)</label>
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center mt-2 flex flex-col items-center justify-center gap-2 hover:border-[#F8510C] transition-colors cursor-pointer group">
                  <Upload size={22} className="text-gray-400 group-hover:text-[#F8510C] transition-colors" />
              ...
              <div className="flex justify-end gap-4 mt-8">
                <button onClick={onClose} className="px-8 py-3 rounded-full bg-[#F3F4F6] text-[#364153] font-bold hover:bg-gray-200 transition-all">
                  Cancel
                </button>
                <button onClick={handleSubmit} className="px-8 py-3 rounded-full bg-[#1E293B] text-white font-bold hover:brightness-110 shadow-lg shadow-[#1E293B]/20 transition-all active:scale-95">
                  Submit Application →
                </button>
              </div>
            </div>
