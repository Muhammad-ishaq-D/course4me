...
const nameField = (label = 'Name') =>
  yup.string().required(`${label} is required`).min(2, `${label} must be at least 2 characters`);
...
export const checkoutBillingSchema = yup.object({
  postcode: yup.string()
    ...
    .matches(/^[A-Z]{1,2}\d[A-Z\d]? ?\d[A-Z]{2}$/i, 'Please enter a valid UK postcode'),
...
export const validateField = async (schema, fieldName, value, allValues = {}) => {
  try {
